...
export default function LabPrintQueue() {
  ...
  // Load printers configuration matching mockup
  const [printers] = useState([
    { id: 'canon-pro300', name: 'Canon PRO-300', status: 'Online', quality: 'High Quality', jobs: 2 },
    { id: 'epson-p700', name: 'Epson SureColor P700', status: 'Online', quality: 'Standard Quality', jobs: 1 },
    { id: 'hp-latex700', name: 'HP Latex 700', status: 'Online', quality: 'Standard Quality', jobs: 1 },
    { id: 'epson-l8050', name: 'Epson L8050', status: 'Offline', quality: 'Maintenance Required', jobs: 0 }
  ]);
  ...
  const filteredJobs = useMemo(() => {
    ...
    if (printerFilter !== 'all') {
      result = result.filter((o, idx) => {
        const assignedPrinter = printers[idx % printers.length];
        return assignedPrinter.name === printerFilter;
      });
    }
  ...
  return (
    <div style={{ padding: '24px 32px', backgroundColor: '#ffffff', minHeight: '100%', fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif", color: '#1e293b', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '12px', marginBottom: '24px' }}>
        ...
        {/* Card 1 */}
        <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
      ...
      <div style={{ width: '100%', overflowX: 'auto', paddingBottom: '16px' }}>
        <div style={{ minWidth: '1200px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          ...
          {/* Filters Bar */}
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', backgroundColor: '#ffffff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '12px', boxShadow: '0 1px 2px rgba(0,0,0,0.01)' }}>
            <div style={{ position: 'relative', flex: 1 }}>
              <span style={{ position: 'absolute', left: '10px', top: '8px', color: '#94a3b8', fontSize: '12px' }}>🔍</span>
              <input
                type="search"
                placeholder="Search by Order ID, Customer..."
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                style={{ width: '100%', padding: '7px 10px 7px 30px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '12.5px', outline: 'none' }}
              />
            </div>
            
            <select value={printerFilter} onChange={(e) => setPrinterFilter(e.target.value)} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '12.5px', backgroundColor: '#fff', outline: 'none' }}>
              <option value="all">All Printers</option>
              {printers.map(p => <option key={p.id} value={p.name}>{p.name}</option>)}
            </select>

            <select value={paperFilter} onChange={(e) => setPaperFilter(e.target.value)} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '12.5px', backgroundColor: '#fff', outline: 'none' }}>
              <option value="all">All Paper Types</option>
              <option value="glossy">Glossy Photo</option>
              <option value="matte">Matte Photo</option>
            </select>

            <select value={priorityFilter} onChange={(e) => setPriorityFilter(e.target.value)} style={{ padding: '7px 10px', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '12.5px', backgroundColor: '#fff', outline: 'none' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: showSidebar ? '1.4fr 0.6fr' : '1fr', gap: '20px', alignItems: 'start' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px', minWidth: 0, overflow: 'hidden' }}>
              ...
              <div style={{ backgroundColor: 'transparent', border: '1px solid #e2e8f0', borderRadius: '10px', overflow: 'hidden', boxShadow: 'none' }}>
                  ...
                  <table style={{ width: '100%', minWidth: '950px', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
                    ...
                    <tbody>
                      {filteredJobs.map((job, idx) => {
                        ...
                        // Map specific printer from mockup array index fallback
                        const assignedPrinter = printers[idx % printers.length];
                          ...
                          <tr key={job.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                            ...
                            <td style={{ padding: '14px' }}>
                              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                <div style={{ width: '48px', height: '48px', border: '1px solid #e2e8f0', borderRadius: '4px', overflow: 'hidden', flexShrink: 0, backgroundColor: '#f8fafc' }}>
                                  {jobItems[0] && getPhotoThumbnail(jobItems[0]) ? (
                                    <img src={getPhotoThumbnail(jobItems[0])} alt="" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                                  ) : (
                                    <div style={{ fontSize: '8px', color: '#94a3b8', padding: '14px 4px' }}>Print</div>
                                  )}
                                </div>
                                <div>
                                  <strong style={{ color: '#0f766e', fontSize: '13px', whiteSpace: 'nowrap' }}>{orderNumber}</strong>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                ...
                <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid #f1f5f9', paddingBottom: '8px' }}>
                    ...
                    <span style={{ fontSize: '11px', color: '#0f766e', fontWeight: 'bold', cursor: 'pointer' }}>View All</span>
                  </div>
                      ...
                      <div key={p.id} style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                        ...
                        <div style={{ flex: 1 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '2px' }}>
                            <span style={{ width: '6px', height: '6px', borderRadius: '50%', backgroundColor: p.status === 'Online' ? '#10b981' : '#64748b' }} />
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{p.status}</span>
                            <span style={{ margin: '0 4px', color: '#cbd5e1' }}>•</span>
                            <span style={{ fontSize: '11px', color: '#64748b' }}>{p.quality}</span>
                          </div>
                        </div>
                ...
                {/* Card B: Supply Levels - Aligned with Icons & Percentages */}
                <div style={{ backgroundColor: '#fff', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px', borderBottom: '1px solid #f1f5f9', paddingBottom: '8px' }}>
                    <span style={{ fontSize: '13px', fontWeight: 'bold', color: '#0f172a' }}>Supply Levels</span>
                    <span style={{ fontSize: '11px', color: '#0f766e', fontWeight: 'bold', cursor: 'pointer' }}>View Details</span>
                  </div>

                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {/* Supply 1 */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12.5px' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#eff6ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', flexShrink: 0 }}>📄</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                          <span>Photo Paper (Glossy)</span>
                          <strong>65%</strong>
                        </div>
                    ...
                    {/* Supply 2 */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12.5px' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#f5f3ff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', flexShrink: 0 }}>📄</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                        ...
                        <div style={{ width: '100%', height: '5px', backgroundColor: '#f1f5f9', borderRadius: '3px', overflow: 'hidden' }}>
                          <div style={{ width: '48%', height: '100%', backgroundColor: '#10b981' }} />
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Supply 3 */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12.5px' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#fdf2f8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', flexShrink: 0 }}>🌀</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    ...
                    {/* Supply 4 */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12.5px' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#fef2f2', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', flexShrink: 0 }}>💧</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                          <span>Black Ink</span>
                          <strong>62%</strong>
                    ...
                    {/* Supply 5 */}
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', fontSize: '12.5px' }}>
                      <div style={{ width: '28px', height: '28px', borderRadius: '50%', backgroundColor: '#ecfdf5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '12px', flexShrink: 0 }}>💧</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                          <span>Color Ink (C, M, Y)</span>
                          <strong>55%</strong>
                        </div>
                        <div style={{ width: '100%', height: '5px', backgroundColor: '#f1f5f9', borderRadius: '3px', overflow: 'hidden' }}>
                          <div style={{ width: '55%', height: '100%', backgroundColor: '#10b981' }} />
                        </div>
                      </div>
                    </div>
