...
export function DeliverySharePublishPanel({
  ...
  const qrSrc = shareUrl ? getQrCodeImageUrl(shareUrl, 200) : '';
  const shareWarning = getShareUrlWarning(shareUrl);
  ...
  const liveBody = (
    <>
          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                if (channel.id !== 'qr') runShareChannel(channel.id);
              }}
            >
      ...
      {shareChannel === 'qr' && shareUrl ? (
        <div className="cd-delivery-share-qr">
          <img src={qrSrc} alt={`QR code for ${title}`} width={200} height={200} />
          {shareWarning ? <p className="cd-delivery-share-hint">{shareWarning}</p> : null}
        </div>
      ) : null}
    </>
  );
  ...
  const introBody = (
    <>
      <p className="cd-delivery-share-hint cd-delivery-share-hint--intro">
        Publish this delivery to get a client link and share it by WhatsApp, email, or QR.
      </p>
      ...
      <p className="cd-delivery-share-footnote">
        Nothing is sent until you pick a channel.
      </p>
    </>
  );
