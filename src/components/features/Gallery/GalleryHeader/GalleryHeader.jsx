...
export function GalleryHeader({ title, opacity, isDark }) {
  return (
    <motion.header 
      style={{ opacity }}
      className={cn(
    ...
    >
      ...
      <div className="flex items-center gap-2 md:gap-6">
        <HeaderAction icon={<Play size={14} fill="currentColor" />} label="Slideshow" hideMobile />
        <HeaderAction icon={<Share2 size={14} />} label="Share" />
        <HeaderAction icon={<Download size={14} />} label="Download" />
        <HeaderAction icon={<Heart size={14} />} label="Favorite" />
      </div>
    </motion.header>
  );
}

function HeaderAction({ icon, label, hideMobile }) {
  return (
    <button className={cn(
