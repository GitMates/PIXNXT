/**
 * Expand a Rekognition face bbox into a square portrait crop (forehead, cheeks, chin).
 * Asymmetric padding: more room above the eyes than below the chin.
 */
export function expandBoundingBoxForPortraitAvatar(bb) {
  ...
  const cropW = Math.max(x2 - x1, 0.01);
  const cropH = Math.max(y2 - y1, 0.01);

  return { Left: x1, Top: y1, Width: cropW, Height: cropH };
}
...
/** 0–1 score: how much of the ideal square portrait crop fits inside the image. */
...
/** Pick the best portrait thumbnail from all indexed faces in each person cluster. */
export function refreshPeopleAvatars(people, metadataRows) {
  const faceEntryById = buildFaceEntryMap(metadataRows);
