...
// [[path]].js
var DEFAULT_API = "https://pixnxt-api.pixnxt.workers.dev";
var CRAWLER_RE = /whatsapp|facebookexternalhit|twitterbot|linkedinbot|telegrambot|discordbot|slackbot|pinterest|googlebot|bingbot/i;
function apiOrigin(env2) {
  const raw = String(env2.API_ORIGIN || env2.VITE_API_URL || DEFAULT_API).trim();
  return raw.replace(/\/+$/, "");
}
...
async function onRequest(context2) {
  ...
  const api = apiOrigin(env2);
  if (path === "/api/crash-report") {
    const crash = String(env2.CRASH_WORKER_URL || env2.VITE_CRASH_WORKER_URL || "").trim().replace(/\/+$/, "");
  ...
  if (path === "/api/crash-query" && request.method === "GET") {
    ...
    const token = String(env2.CRASH_ADMIN_TOKEN || env2.ADMIN_TOKEN || "").trim();
...
// ../.wrangler/tmp/pages-r8eyTP/functionsRoutes-0.008240729208182085.mjs
var routes = [
  {
    routePath: "/:path*",
    mountPath: "/",
